...
public class GestorLliga {
			...
			while ((linia = br.readLine()) != null) {
				...
				try {
					...
					if (nomLocal.equalsIgnoreCase(nomVisitant)) {
						System.err.println(
								"Error a la línia " + comptadorLinia + ": Un equip no pot jugar contra si mateix.");
						continue;
					}
	...
	private Equip cercarEquip(String nom) {
		return equipsUnics.stream().filter(e -> e.getNom().equalsIgnoreCase(nom)).findFirst().orElse(null);
	}
